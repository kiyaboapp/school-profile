...
import { getCouncilBySlug, getCouncilSelectionsOverview, getAvailableYears } from '@/lib/api';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!council) {
    return {
      title: 'Halmashauri Haupatikani | ShuleYetu',
      description: 'Halmashauri haupatikani',
    };
  }

  return {
    title: `${council.council_name} - Uchaguzi wa Kidato cha Tano ${year} | ShuleYetu`,
    description: `Takwimu za uchaguzi wa kidato cha tano kwa halmashauri ya ${council.council_name} mwaka ${year}. Angalia idadi ya wanafunzi waliochaguliwa, mchanganyiko, na shule walizoingia.`,
    openGraph: {
      title: `${council.council_name} - Uchaguzi wa Kidato cha Tano ${year}`,
      description: `Takwimu za uchaguzi wa kidato cha tano kwa halmashauri ya ${council.council_name} mwaka ${year}.`,
      type: 'website',
    },
  };
}

export default async function CouncilSelectionsPage({ params }: PageProps) {
  const { slug, year: yearStr } = await params;
  const year = parseInt(yearStr, 10);
  
  const [council, overview, availableYears] = await Promise.all([
    getCouncilBySlug(slug),
    getCouncilSelectionsOverview(0, year), // TODO: pass actual councilId
    getAvailableYears(),
  ]);
  ...
  const totalStudents = overview?.totalStudents || 3542;
  const totalFemale = overview?.totalFemale || 1923;
  const totalMale = overview?.totalMale || 1619;
  ...
  const breadcrumbs: { label: string; href: string }[] = [
    ...
    { label: council.region?.region_name || 'Mkoa', href: `/mkoa/${council.region?.slug || 'dar-es-salaam'}/uchaguzi/${year}` },
    { label: council.council_name, href: `/halmashauri/${slug}/uchaguzi/${year}` },
  ];
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <header className="mt-6 mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <YearSelector
              ...
              baseUrl={`/halmashauri/${slug}/uchaguzi`}
            />
          </div>
        </header>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-3xl font-bold text-purple-600">{overview?.schoolsReceiving || 18}</p>
          </div>
        </section>
        ...
        <section className="bg-white rounded-lg shadow p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <GeographicScopeCard
              wardScope={Math.round(totalStudents * 0.25)}
              councilScope={Math.round(totalStudents * 0.45)}
              regionalScope={Math.round(totalStudents * 0.20)}
              outsideRegion={Math.round(totalStudents * 0.10)}
                ...
                {topSchools.length > 0 ? (
                  topSchools.map((school: any, idx: number) => (
                    <tr key={school.centre_number} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900">{school.ward?.ward_name || '-'}</td>
